"""Attach newest processed build to Plainly 1.0, create a review submission with version + subs + group version, submit.
Run: cd landed/.credentials && PYTHONPATH=. python3 ~/workspace/plainly/store/asc_submit.py [--dry-run]"""
import asc, json, sys, time, os
APP='6809042236'; GROUP=os.environ['PLAINLY_GROUP']; SUBS=os.environ['PLAINLY_SUBS'].split(','); DRY='--dry-run' in sys.argv
